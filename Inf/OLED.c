...
static OLED_Driver_t s_default_driver = {0}; // 内部静态实例
OLED_Driver_t* g_oled_driver = &s_default_driver;
...
static void OLED_WritePacket(uint8_t control, const uint8_t *payload, uint16_t payload_len)
{
	uint8_t tx_buf[129];

	if (g_oled_driver == NULL || g_oled_driver->OLED_I2C_WriteFunc == NULL)
	{
		return;
	}
	...
	while (payload_len > 0)
	{
		uint16_t chunk_len = (payload_len > 128) ? 128 : payload_len;
		tx_buf[0] = control;
		memcpy(&tx_buf[1], payload, chunk_len);
		g_oled_driver->OLED_I2C_WriteFunc(tx_buf, chunk_len + 1);
...
{
	OLED_WritePacket(0x00, &Command, 1);
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	OLED_WritePacket(0x40, Data, Count);
}

// OLED注册驱动（只注册I2C写函数）
void OLED_RegisterDriver(void (*OLED_I2C_WriteFunc)(const uint8_t *Data, uint16_t Count))
{
	g_oled_driver->OLED_I2C_WriteFunc = OLED_I2C_WriteFunc;
}
